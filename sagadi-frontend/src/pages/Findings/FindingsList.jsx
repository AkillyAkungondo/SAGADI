...
export const FindingsList = () => {
  ...
  const carregarFindings = async () => {
    try {
      ...
      if (filtros.apenas_atrasados) {
        params.atrasados = true;
      }

      if (filtros.apenas_abertos) {
        params.status_not = "encerrado";
      }
      ...
      const concluidos =
        data?.filter((f) => f.status === "encerrado").length || 0;
  ...
  const carregarPesquisasSalvas = () => {
    ...
    if (salvas) {
      setPesquisasSalvas(JSON.parse(salvas));
    }
  };
  ...
  const getPrioridadeChip = (prioridade) => {
    ...
    return (
      <Chip
        label={prioridade?.toUpperCase() || "N/A"}
        color={cores[prioridade] || "default"}
        size="small"
      />
    );
  };
  ...
  const paginatedFindings = findingsFiltrados.slice(
    page * rowsPerPage,
  ...
  return (
    <Layout title="Findings">
      ...
      >
        <Box>
          ...
          <Typography variant="body2" color="textSecondary">
            {estatisticas.total} findings encontrados • {estatisticas.abertos}{" "}
            abertos • {estatisticas.atrasados} atrasados
          </Typography>
        </Box>

        <Box sx={{ display: "flex", gap: 1 }}>
          <Tooltip title="Exportar CSV">
            <IconButton onClick={exportarParaCSV}>
              <DownloadIcon />
            </IconButton>
          </Tooltip>

          <Tooltip title="Relatório PDF">
            <IconButton onClick={gerarRelatorioPDF}>
              <PdfIcon />
            </IconButton>
          </Tooltip>
          ...
          {(isInspetor || isAdmin) && (
            <Button
              variant="contained"
              startIcon={<AddIcon />}
              onClick={() => navigate("/findings/novo")}
            >
              Novo Finding
            </Button>
          )}
      ...
      <Menu
        anchorEl={menuPesquisas}
        open={Boolean(menuPesquisas)}
        onClose={() => setMenuPesquisas(null)}
      >
        {pesquisasSalvas.length === 0 ? (
          ...
          pesquisasSalvas.map((pesquisa) => (
            <MenuItem key={pesquisa.id}>
              ...
              <ListItemText
                primary={pesquisa.nome}
                secondary={new Date(pesquisa.data).toLocaleDateString()}
                onClick={() => carregarPesquisaSalva(pesquisa)}
              />
              <IconButton
                size="small"
                edge="end"
                onClick={() => removerPesquisaSalva(pesquisa.id)}
              >
          ...
          <Grid item>
            <Badge badgeContent={filtrosAtivos} color="primary">
              <Button
                ...
                onClick={() =>
                  document
                    .getElementById("filtros-avancados")
      ...
      {/* Filtros Avançados */}
      <Card sx={{ mb: 3 }} id="filtros-avancados">
        <Accordion>
            ...
            <Grid container spacing={3}>
              ...
              <Grid item xs={12} md={3}>
                <FormControl fullWidth size="small">
                  ...
                  >
                    ...
                    <MenuItem value="parte1_concluida">
                      Aguardando Operador
                    </MenuItem>
                    <MenuItem value="parte2_concluida">
              ...
              <Grid item xs={12} md={3}>
                <FormControl fullWidth size="small">
                  ...
                  >
                    ...
                    <MenuItem value="vencimento_asc">
                      Vencimento (próximo)
                    </MenuItem>
                    <MenuItem value="vencimento_desc">
                      Vencimento (distante)
                    </MenuItem>
                    <MenuItem value="prioridade_desc">
                      Prioridade (maior)
                    </MenuItem>
                    <MenuItem value="prioridade_asc">
                      Prioridade (menor)
                    </MenuItem>
                  </Select>
                </FormControl>
              ...
              <Grid item xs={12}>
                <TextField
                  ...
                  onChange={handleFiltroChange}
                  placeholder="Digite palavras para buscar na descrição..."
                />
              </Grid>

              {/* Linha 4 - Filtros booleanos */}
              <Grid item xs={12}>
                <Paper variant="outlined" sx={{ p: 2 }}>
                  ...
                  <FormGroup row>
                    {(isInspetor || isOperador) && (
                      <FormControlLabel
                        control={
                          <Checkbox
                            name="apenas_meus"
                            checked={filtros.apenas_meus}
                            onChange={handleFiltroChange}
                          />
                        }
                        label="Apenas meus findings"
                      />
                    )}
                    <FormControlLabel
                      control={
                        <Checkbox
                          name="apenas_abertos"
                          checked={filtros.apenas_abertos}
                          onChange={handleFiltroChange}
                        />
                      }
                    ...
                    <FormControlLabel
                      control={
                        <Checkbox
                          name="apenas_atrasados"
                          checked={filtros.apenas_atrasados}
                          onChange={handleFiltroChange}
                        />
                      }
              ...
              {/* Linha 5 - Slider de prazo */}
              <Grid item xs={12}>
                <Typography gutterBottom>
                  Prazo máximo (dias): {filtros.prazo_maximo}
                </Typography>
                <Slider
                  value={filtros.prazo_maximo}
      ...
      <TableContainer component={Paper}>
        <Table size="small">
          <TableHead>
            <TableRow>
              <TableCell>
                <TableSortLabel
                  active={orderBy === "numero_processo"}
                  direction={orderBy === "numero_processo" ? order : "asc"}
                  onClick={() => handleRequestSort("numero_processo")}
                >
                  Nº Processo
                </TableSortLabel>
              </TableCell>
              <TableCell>
                <TableSortLabel
                  active={orderBy === "aerodromo_id"}
                  direction={orderBy === "aerodromo_id" ? order : "asc"}
                  onClick={() => handleRequestSort("aerodromo_id")}
                >
                  Aeródromo
                </TableSortLabel>
              </TableCell>
              ...
              <TableCell>
                <TableSortLabel
                  active={orderBy === "data_inspecao"}
                  direction={orderBy === "data_inspecao" ? order : "asc"}
                  onClick={() => handleRequestSort("data_inspecao")}
                >
                  Data
                </TableSortLabel>
              </TableCell>
                  ...
                  <Fade in={true}>
                    <Box sx={{ py: 4 }}>
                      ...
                      <Typography variant="caption" color="textSecondary">
                        Tente ajustar os filtros ou criar um novo finding
                      </Typography>
                    </Box>
                  </Fade>
                </TableCell>
                ...
                return (
                  <Zoom in={true} key={finding.id}>
                    ...
                    >
                      ...
                      <TableCell>
                        {getPrioridadeChip(finding.prioridade)}
                      </TableCell>
                      <TableCell>
                        <Box
                          sx={{
                        ...
                        >
                          ...
                          <Tooltip title="Ver Documento">
                            <IconButton
                              size="small"
                              onClick={() =>
                                navigate(`/findings/${finding.id}/documento`)
                              }
                              color="primary"
                            >
                            ...
                            finding.status === "rascunho" && (
                              <Tooltip title="Editar">
                                <IconButton
                                  size="small"
                                  onClick={() =>
                                    navigate(`/findings/editar/${finding.id}`)
                                  }
                                >
        ...
        <TablePagination
          ...
          component="div"
          count={findingsFiltrados.length}
      ...
      >
        <Typography variant="body2" color="textSecondary">
          Mostrando {paginatedFindings.length} de {findingsFiltrados.length}{" "}
          findings
        </Typography>
        ...
        <Box sx={{ display: "flex", gap: 2 }}>
          ...
          <Chip
            icon={<WarningIcon />}
            label={`${estatisticas.abertos} abertos`}
            size="small"
            color="warning"
            variant="outlined"
          />
