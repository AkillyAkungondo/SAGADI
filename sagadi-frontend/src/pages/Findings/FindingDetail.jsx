...
export const FindingDetail = () => {
  ...
  const carregarFinding = async () => {
    try {
      ...
      // Pré-carregar dados de resposta se existirem
      ...
      // Pré-carregar dados de avaliação se existirem
      if (data.comments_cap) {
        setAvaliacaoData({
          comments_cap: data.comments_cap || "",
          evaluation_actions: data.evaluation_actions || "",
          data_aplicacao_acoes: data.data_aplicacao_acoes
            ? new Date(data.data_aplicacao_acoes).toISOString().split("T")[0]
            : "",
          resolved_satisfactorily: data.resolved_satisfactorily || false,
          progress_documented: data.progress_documented?.length
            ? data.progress_documented
  ...
  const getStepFromStatus = (status) => {
    switch (status) {
      case "rascunho":
      case "parte1_concluida":
        return 1;
      case "aguarda_parte2":
      case "parte2_concluida":
        return 2;
  ...
  const removerAcao = (index) => {
    if (respostaData.acoes_corretivas.length > 1) {
      const novasAcoes = respostaData.acoes_corretivas.filter(
        (_, i) => i !== index,
      );
      setRespostaData((prev) => ({ ...prev, acoes_corretivas: novasAcoes }));
    }
  ...
  const removerProgressDoc = (index) => {
    if (avaliacaoData.progress_documented.length > 1) {
      const novosProgress = avaliacaoData.progress_documented.filter(
        (_, i) => i !== index,
      );
      setAvaliacaoData((prev) => ({
        ...prev,
    ...
    <Layout title={`Finding ${finding.numero_processo}`}>
      ...
      >
        ...
        <Box sx={{ display: "flex", gap: 1 }}>
          {/* Botão para ver o documento oficial */}
          <Tooltip title="Ver Documento Oficial">
            <IconButton
              onClick={() => navigate(`/findings/${id}/documento`)}
              color="primary"
            >
              <DescriptionIcon />
            </IconButton>
      ...
      <Paper sx={{ p: 3, mb: 3 }}>
        <Stepper
          activeStep={getStepFromStatus(finding.status)}
          alternativeLabel
        >
          <Step>
      ...
      <Paper sx={{ p: 3, mb: 3 }}>
        <Grid container spacing={2}>
          ...
          <Grid item xs={12} md={2}>
            ...
            <Box sx={{ mt: 0.5 }}>{getStatusChip(finding.status)}</Box>
          </Grid>
          <Grid item xs={12} md={2}>
            <Typography variant="caption" color="textSecondary">
              Prioridade
            ...
            <Typography variant="body1">
              {finding.prioridade?.toUpperCase()}
            </Typography>
          </Grid>
          <Grid item xs={12} md={2}>
            <Typography variant="caption" color="textSecondary">
            ...
            <Typography
              variant="body1"
              color={
                finding.data_vencimento &&
                new Date(finding.data_vencimento) < new Date()
                  ? "error"
                  : "inherit"
              }
            >
              {finding.data_vencimento
                ? new Date(finding.data_vencimento).toLocaleDateString()
                : "N/A"}
            </Typography>
          </Grid>
          <Grid item xs={12} md={3}>
      ...
      <Paper sx={{ p: 3 }}>
        <Tabs value={tabValue} onChange={handleTabChange} sx={{ mb: 3 }}>
          ...
          <Tab
            label="Parte 2 - Resposta"
            disabled={
              !finding.parte_2_concluida_em &&
              finding.status === "parte1_concluida"
            }
          />
          <Tab
            label="Parte 3 - Avaliação"
            disabled={finding.status !== "encerrado" && !finding.comments_cap}
          />
          <Tab label="Anexos" />
        </Tabs>
            ...
            <Grid container spacing={3}>
              ...
              <Grid item xs={12} md={6}>
                ...
                <Typography variant="body1">
                  ...
                  {finding.finding_level === 1
                    ? "Baixo"
                    : finding.finding_level === 2
                      ? "Médio"
                      : "Alto"}
                </Typography>
              </Grid>
              ...
              {finding.parte_1_concluida_em && (
                <Grid item xs={12}>
                  <Typography variant="caption" color="textSecondary">
                    Concluída em:{" "}
                    {new Date(finding.parte_1_concluida_em).toLocaleString()}
                  </Typography>
                </Grid>
              )}
            </Grid>
              ...
              <Grid container spacing={3}>
                ...
                {finding.operador_resposta && (
                  <Grid item xs={12}>
                    <Typography variant="caption" color="textSecondary">
                      Respondido por:{" "}
                      {finding.operador_resposta.nome_completo} em{" "}
                      {new Date(
                        finding.parte_2_concluida_em,
                      ).toLocaleString()}
                    </Typography>
            ...
            {finding.comments_cap || finding.evaluation_actions ? (
              <Grid container spacing={3}>
                <Grid item xs={12}>
                  ...
                  >
                    <Typography variant="body1">
                      {finding.comments_cap || "Não especificado"}
                    </Typography>
                  </Card>
                </Grid>
                ...
                <Grid item xs={12}>
                  ...
                  >
                    <Typography variant="body1">
                      {finding.evaluation_actions || "Não especificado"}
                    </Typography>
                  </Card>
                </Grid>
                      ...
                      <Table size="small">
                        <TableHead>
                          <TableRow>
                            ...
                            <TableCell>Data de Revisão</TableCell>
                          </TableRow>
                        </TableHead>
                        <TableBody>
                          {finding.progress_documented.map((item, index) => (
                            <TableRow key={index}>
                ...
                <Grid item xs={12} md={6}>
                  ...
                  <Typography variant="body1">
                    {finding.data_aplicacao_acoes ? new Date(finding.data_aplicacao_acoes).toLocaleDateString() : 'Não definida'}
                  </Typography>
                </Grid>
                ...
                <Grid item xs={12} md={6}>
                  ...
                  <Typography variant="body1">
                    {finding.resolved_satisfactorily ? 'Sim' : 'Não'}
                  </Typography>
                </Grid>
                ...
                {finding.inspector_assinatura && (
                  <Grid item xs={12}>
                    ...
                    <Typography variant="caption" color="textSecondary">
                      {finding.data_assinatura ? new Date(finding.data_assinatura).toLocaleString() : ''}
                    </Typography>
                  </Grid>
                )}
              </Grid>
                  ...
                  <TableBody>
                    {finding.anexos.map((anexo) => (
                      <TableRow key={anexo.id}>
                        ...
                        <TableCell>
                          {anexo.uploaded_por?.nome_completo}
                        </TableCell>
      ...
      >
        ...
        <DialogContent>
          ...
          >
            <TextField
              ...
              value={respostaData.observacoes_operador}
            ...
            <TextField
              ...
              name="root_causes"
              label="Root Cause(s)"
              value={respostaData.root_causes}
              onChange={handleRespostaChange}
            />
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={12} md={5}>
                    <TextField
                      ...
                      onChange={(e) =>
                        handleAcaoChange(index, "start_date", e.target.value)
                      }
                  ...
                  <Grid item xs={12} md={5}>
                    <TextField
                      ...
                      value={acao.due_date}
                      onChange={(e) =>
                        handleAcaoChange(index, "due_date", e.target.value)
                      }
                      InputLabelProps={{ shrink: true }}
                    />
                  </Grid>
                  ...
                  {respostaData.acoes_corretivas.length > 1 && (
                    <Grid item xs={12}>
                      <Button
                        size="small"
                        color="error"
            ...
            {respostaData.acoes_corretivas.length < 4 && (
              <Button
                variant="outlined"
        ...
        <DialogActions>
          <Button onClick={() => setOpenResposta(false)}>Cancelar</Button>
          <Button
        ...
        <DialogContent>
          ...
          >
            <TextField
              ...
              rows={3}
              name="comments_cap"
              label="Comments on CAP"
              value={avaliacaoData.comments_cap}
            ...
            <TextField
              ...
              label="Evaluation of Actions"
              value={avaliacaoData.evaluation_actions}
              onChange={handleAvaliacaoChange}
            />
            ...
            {avaliacaoData.progress_documented.map((item, index) => (
              <Grid container spacing={2} key={index}>
                ...
                {avaliacaoData.progress_documented.length > 1 && (
            ...
            {avaliacaoData.progress_documented.length < 4 && (
              <Button
                variant="outlined"
            ...
            <TextField
              ...
              value={avaliacaoData.data_aplicacao_acoes}
              onChange={handleAvaliacaoChange}
              InputLabelProps={{ shrink: true }}
            />
            ...
            <FormControlLabel
              control={
                <Checkbox
                  ...
                  checked={avaliacaoData.resolved_satisfactorily}
                  onChange={handleAvaliacaoChange}
                />
              }
              label="Resolvido Satisfatoriamente"
            />
          </Box>
        </DialogContent>
        <DialogActions>
      ...
      >
        ...
        <DialogActions>
          ...
          >
            {submitting ? <CircularProgress size={24} /> : "Fazer Upload"}
          </Button>
        </DialogActions>
      </Dialog>
    </Layout>
